...
class ReactUeditor extends React.Component {
  ...
  constructor () {
    ...
    this.registerImageUpload = this.registerImageUpload.bind(this)
  }

  componentDidMount () {
    let props = this.props
    console.log('111',`${props.ueditorPath}`)
  ...
  // uditor 自定义按钮的方式uditor 自定义按钮的方式
  registerImageUpload () {
    console.log('触发')
    window.UE.registerUI('imageUpload', (editor, uiName) => {
      ...
      let btn = new window.UE.ui.Button({
        ...
        onclick: () => {
          if (this.props.isManyImg) {
            //多图片上传
            this.props.dispatch({
              type: 'filterDetail/changeModalStatus',
              payload: {
                showModal: true,
              },
            })
            this.props.getUeditor && this.props.getUeditor(ueditor)
            return
          }
  ...
  initEditor () {
    ...
    ueditor = window.UE.getEditor('container1')
    ...
    window.ueditor = ueditor // 全局添加ueditor，用于点击tap销毁
  }
  ...
  render () {
    return (
      <div>
        ...
        {/* <input type="file" accept="image/*" id="tempfileInput" onChange={this.uploadImage} style={{ visibility: 'hidden' }} /> */}
      </div>
    )
  }
}
